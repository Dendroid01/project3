...
export default function LoginPage() {
    ...
    return (
        <div className="min-h-screen py-16 px-5 flex flex-col items-center justify-center bg-[#0B1326]">
            ...
            <main className="w-full p-6 bg-[#171F31] shadow-[0_8px_32px_#09101F] rounded-4xl border border-[#2d3648] mb-4">
                <h2 className="text-[#DAE2FD] font-bold text-2xl text-center mb-1.5">Login</h2>
                <LoginForm onSubmit={()=>{}} isLoading={isLoading} serverError={error} />
            </main>
            <p className="text-center text-[#BEC8D2] font-regular">
                Don't have an account? <a href="/register" className="text-[#89CEFF] font-bold hover:underline">Create Account</a>
            </p>
        </div>
    );
}